...
1. `B2_RegionX_all_QCpass_479.ipynb` analyses all `primary_include_revised` cells, derives a preliminary lymphoid-enriched core, retains only its largest non-noise DBSCAN cluster (`eps=80`, `minPts=10`) as the lymph-node domain, tests boundary sensitivity, and freezes `lymph_node_domain_manifest.tsv.gz`.
...
The default clustering algorithm is Seurat Louvain algorithm 1. Set `SCWAT_CLUSTER_ALGORITHM=4` only when the controlled HPC R library contains `leidenbase`; the selected algorithm is written to provenance. `diptest`, `mclust`, and `CellChat` are optional diagnostics and are explicitly reported as skipped when unavailable. Spatial CellChat uses normalized Xenium expression, aligned centroids and cell-area-derived spatial factors for the five strongest KNN-selected neighbour types in each Eosinophil-state direction. The reported significant subset is restricted to outgoing Eosinophil-state interactions with that state's top-five neighbours; the complete communication table is retained for audit. The adapter detects whether the installed CellChat uses the current `spatial.factors` API or the legacy `scale.factors` API. Results are labelled exploratory within-section inference, not mouse-level evidence or proof of signalling. The complete all-age Wang reference is primary for transfer, Eosinophil calibration and joint CCA integration; the 2.5-month transfer remains an age-matched sensitivity comparison. The joint embedding is a concordance diagnostic only.

All tabular exports are normalized to rectangular atomic columns before writing. Matrix- or list-valued columns returned by optional packages are expanded or serialized, preventing the base-R `write.table()` dimname/array-extent failure while retaining the original evidence in auditable TSV form.
