    ...
    def test_twelve_region_branch_notebooks_are_clean_and_region_locked(self):
        for region in range(1, 5):
            for branch in BRANCHES:
                ...
                self.assertIn("plot_target_overlay", text)
                ...
                self.assertIn('cols = c("#D9D9D9", "#5A2F5E")', text)
                self.assertIn("top_n = 5L", text)
                self.assertIn("signed_fraction", text)
                self.assertIn("proximity_difference", text)
