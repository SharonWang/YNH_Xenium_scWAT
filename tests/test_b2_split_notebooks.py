    ...
    def test_twelve_region_branch_notebooks_are_clean_and_region_locked(self):
        for region in range(1, 5):
            for branch in BRANCHES:
                ...
                self.assertIn("validate_disjoint_spatial_pools", text)
                self.assertIn("validate_neighbour_distances", text)
                self.assertIn("optional_packages", text)
                self.assertIn("RUN_DIPTEST", text)
                self.assertIn("RUN_MCLUST", text)
                self.assertIn("RUN_LR_COEXPRESSION", text)
                self.assertIn("nearest_cell_type = character()", text)
                self.assertIn("mean_expression_product = numeric()", text)
